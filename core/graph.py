...
class DependencyGraph:
    ...
    def topological_sort(self) -> list[str]:
        ...
        # Construir grafo invertido para topological sort correto
        # Se A depende de B, B deve vir antes de A
        adj: dict[str, set[str]] = defaultdict(set)
        in_deg: dict[str, int] = {node: 0 for node in self._nodes}

        for node in self._nodes:
            for dep in self._graph[node]:
                adj[dep].add(node)  # dep -> node (dep deve vir antes)
                in_deg[node] += 1
        ...
        if len(result) != len(self._nodes):
            # Encontrar o ciclo para mensagem de erro
